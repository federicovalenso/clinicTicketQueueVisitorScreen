...
TicketsPlayer::TicketsPlayer(QObject *parent) : QObject(parent) {
...
QMediaPlayer::State TicketsPlayer::state() const { return player->state(); }

void TicketsPlayer::tryToPlay() {
  if (tickets.size() > 0) {
    playlist->addMedia(QUrl(SOUND_RES_PREFIX + TICKET_FILE_NAME));
    Ticket ticket = tickets.back();
    for (const auto &token : TicketNumberParsing::parse(ticket.ticket_number)) {
